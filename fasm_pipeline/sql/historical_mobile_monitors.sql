...
changes AS (
  ...
  FROM pwfsl_historical.mobile_monitors h
...
INSERT INTO pwfsl_historical.mobile_monitors 
  (unit_id, latitude, longitude, utc_ts, local_ts, timezone, raw_pm25, nowcast, aqi,
   site_name, deployment_type, aqsid, full_aqsid, status, provider, geom)
SELECT 
  unit_id, latitude, longitude, utc_ts, local_ts, timezone, raw_pm25, nowcast, aqi,
  site_name, deployment_type, aqsid, full_aqsid, status, provider, geom
FROM source_data
...
DO UPDATE SET
  ...
  geom = EXCLUDED.geom,
  created_at = NOW()
WHERE pwfsl_historical.mobile_monitors.raw_pm25 IS DISTINCT FROM EXCLUDED.raw_pm25
   OR pwfsl_historical.mobile_monitors.nowcast IS DISTINCT FROM EXCLUDED.nowcast
   OR pwfsl_historical.mobile_monitors.status IS DISTINCT FROM EXCLUDED.status;